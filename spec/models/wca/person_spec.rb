...
describe WCA::Person do
  ...
  it "gets Stefan Pochmann's best single for 3x3x3" do
    VCR.use_cassette "person/2003POCH01/single" do
      result = WCA::Person.find("2003POCH01").best_single_for("333") # TODO don't go through Person, Result also includes cached name and stuff
      result.first.average.should == 1721
      result.first.value4.should == 956
    end
  end

  it "gets Stefan Pochmann's best average for magic" do
    VCR.use_cassette "person/2003POCH01/average" do
      result = WCA::Person.find("2003POCH01").best_average_for("magic")
      result.first.average.should == 166
      result.first.value1.should == 143
    end
  end
end
